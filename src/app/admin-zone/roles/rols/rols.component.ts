...
export class RolsComponent implements OnInit {
  ...
  rols: ModelRols[]
  ngOnInit() {
    this._rolService.rolObtener().subscribe((r: any) => {
      this.rols = r
  ...
  displayedColumns: string[] = ['id', 'nombre', 'descripcion', 'activo', 'fechaInsertada', 'fechaUltMod', '_'];
  ...
  edit(item) {
    ...
    this._route.navigateByUrl('/admin/rols/form', item)
